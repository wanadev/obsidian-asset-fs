...
var AssetFs = Class.$extend({
    ...
    addIndexFromUrl: function(indexUrl, rootUrl) {
        ...
        if (!this.$data.dataFetcher) {
            return Q.reject(new Error("MissingDataFetcher"));
        }
    ...
    getAssetAsImage: function(assetPath) {
        return this.getAssetAsBlobUrl(assetPath)
            .then(helpers.imageFromUrl);
    },
